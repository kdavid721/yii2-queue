...
abstract class Queue extends \yii\base\Component
{
    ...
    /**
     * @var int multi threading
     */
    public $maxProcesses = 1;

    /**
     * @var int interval to check if processes have finished
     */
    public $waitSecondsIfNoProcesses = 5;

    /**
     * @var bool whether we want a kill signal to propagate to threads
     * or if we want them to finish on their own, treat carefully
     */
    public $propagateSignals = false;

    /**
     * @var int idle timeout on each single process
     * e.g. timeout process after x seconds if there is no output
     */
    public $idleTimeout = null;

    /**
     * @var int $sleepTimeout sleep timeout to be slept after each process is launched
     * forced to a minimum of one in order to register the process after its run
     */
    ...
    /**
     * @var obj ProcessRunner the process runner dependency
     */
    public $processRunner = null;

    /**
     * @var str $command command to run single jobs process
     */
    public $command = 'queue/run';

    /**
     * register the process runner depndency
     * @param ProcessRunner $processRunner
     * @param array $config additional params
     */
    public function __construct(ProcessRunner $processRunner, array $config = [])
    {
        parent::__construct($config);
        $this->processRunner = $processRunner->setQueue($this);
    }
